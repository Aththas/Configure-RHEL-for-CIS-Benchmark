## 1. If found, delete the line which contains 'Banner'
    $ sudo LC_ALL=C sed -i "/^\s*Banner\s\+/Id" /etc/ssh/sshd_config
...
## 4. Insert at the beginning of the file
    $ sudo echo "Banner /etc/issue.net" | sudo tee /etc/ssh/sshd_config
