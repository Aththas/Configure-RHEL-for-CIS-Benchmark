## 1. If found, delete the line which contains 'LogLevel'
    $ sudo LC_ALL=C sed -i "/^\s*LogLevel\s\+/Id" /etc/ssh/sshd_config
...
## 4. Insert at the beginning of the file
    $ sudo echo "LogLevel VERBOSE" | sudo tee /etc/ssh/sshd_config
