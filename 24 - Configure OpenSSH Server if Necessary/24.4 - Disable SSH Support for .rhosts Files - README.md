## 1. If found, delete the line which contains 'IgnoreRhosts'
    $ sudo LC_ALL=C sed -i "/^\s*IgnoreRhosts\s\+/Id" /etc/ssh/sshd_config
...
## 4. Insert at the beginning of the file
    $ sudo echo "IgnoreRhosts yes" | sudo tee /etc/ssh/sshd_config
